...
# 注意：这里的 probe 是在 precomputed image-only features 上训练出来的
probe_path = "/workspace/probing/Probing/src/multi_field_probe_on_features.pt"
output_jsonl_path = "/workspace/probing/Probing/src/probing_predictions_all_multi_fields_image_only.jsonl"
...
# ===================== Probing 预测 + 存 jsonl（多字段，image-only） =====================
@torch.no_grad()
def run_probing_and_dump_multi_fields(
    ...
    """
    对给定 samples 跑一遍 probing（image-only 版本）：
      - 每张图只构造一次 dummy 文本（仅含 <image>），抽一份特征
      - 通过多头 probe 一次性预测所有字段
    ...
    for idx, s in enumerate(tqdm(samples, desc="Probing (image-only)")):
        # 1. 读图 + resize
        ...
        # 2. 构造与 precompute 一致的 dummy 文本：只包含一个 <image> 占位
        dummy_text = image_token
        ...
        # 3. 抽特征（与 precompute / train_probe 时的 image-only 设定对齐）
        features = feature_extractor(enc)  # (1, D)

        # 4. 一次前向，多头同时预测
        logits_dict = probe(features)      # {field: (1, num_classes_for_field)}
        ...
        for field in active_fields:
            ...
            logits = logits_dict[field]  # (1, C_field)
...
def main():
    ...
    # ===== 2. 构建特征提取器（与 precompute / train_probe 保持一致）=====
